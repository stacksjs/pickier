...
function hasBlockOnOrNextLine(lines: string[], i: number, afterIndex: number): boolean {
  const line = lines[i]
  if (line.slice(afterIndex).includes('{')) return true
...
function requiresBracesHeuristic(lines: string[], i: number, afterIndex: number): boolean {
  ...
  if (/(?:\bif\b|\bfor\b|\bwhile\b|\bdo\b)/.test(tail)) return true
  return false
}
        ...
        if (chainHasBlock || chainNeeds) {
          ...
          while (true) {
            ...
            if (elseIf2) {
              ...
              if (!hb2) issues.push({ filePath: ctx.filePath, line: k + 1, column: Math.max(1, t.indexOf('else') + 1), ruleId: 'style/curly', message: 'Expect curly brackets', severity: 'warning' })
              cur = k
              continue
            }
            else {
...
export const _internal_detectUnnecessaryElseBraces = (text: string, filePath: string): Array<{ line: number; column: number; message: string }> => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (bracePos < 0) {
      const j = (function nextNonEmpty(start: number): number { for (let k = start; k < lines.length; k++) { if (lines[k].trim() !== '') return k }; return -1 }) (i + 1)
      if (j < 0) continue
    ...
    if (inner.length === 1 && endLine === startLine + 2) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        if (t2 === '' || t2 === '}' || /^\/\//.test(t2)) continue
        if (!/^else\b/.test(t2)) break
        // find block start for this branch
        let after2 = lines[j].indexOf('else') + 'else'.length
        const m2 = t2.match(/^else\s+if\s*\([^)]*\)/)
        if (m2) after2 = lines[j].indexOf(m2[0]) + m2[0].length
        let start2 = j
        let pos2 = lines[j].indexOf('{', after2)
        if (pos2 < 0) {
          const nn = (function nextNonEmpty(start: number): number { for (let k2 = start; k2 < lines.length; k2++) { if (lines[k2].trim() !== '') return k2 }; return -1 })(j + 1)
          if (nn < 0) break
        ...
        if (chainHasMulti) break
      }
      // direct neighbor else check (additional safeguard)
      if (!chainHasMulti && kind === 'if') {
        const nn = (function nextNonEmpty(start: number): number { for (let k3 = start; k3 < lines.length; k3++) { const tt = lines[k3].trim(); if (tt !== '' && tt !== '}' && !/^\/\//.test(tt)) return k3 }; return -1 })(endLine + 1)
        if (nn >= 0 && /^else\b/.test(lines[nn].trim())) {
          let after3 = lines[nn].indexOf('else') + 'else'.length
          const m3 = lines[nn].trim().match(/^else\s+if\s*\([^)]*\)/)
          if (m3) after3 = lines[nn].indexOf(m3[0]) + m3[0].length
          let start3 = nn
          let pos3 = lines[nn].indexOf('{', after3)
          if (pos3 < 0) {
            const mm = (function nextNonEmpty(start: number): number { for (let k4 = start; k4 < lines.length; k4++) { if (lines[k4].trim() !== '') return k4 }; return -1 })(nn + 1)
