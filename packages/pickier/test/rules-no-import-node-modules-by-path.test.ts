...
const create = (c: string) => { const p = resolve(__dirname, `tmp-no-nm-path-${Date.now()}-${Math.random().toString(36).slice(2)}.ts`); writeFileSync(p, c); tempFiles.push(p); return p }
