...
function create(c: string) { const p = resolve(__dirname, `tmp-import-dedupe-${Date.now()}-${Math.random().toString(36).slice(2)}.ts`); writeFileSync(p, c); tempFiles.push(p); return p }
function cleanup() {
