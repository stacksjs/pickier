...
function tmp(): string {
  return mkdtempSync(join(tmpdir(), 'pickier-rule-sort-objects-'))
}

describe('pickier/sort-objects', () => {
  it('flags unsorted object keys (alphabetical asc)', async () => {
    const dir = tmp()
    const file = 'a.ts'
    const src = [
      'const event = {',
  ...
  it('respects partitionByNewLine (groups not sorted across blank lines)', async () => {
    const dir = tmp()
    const file = 'b.ts'
    const src = [
      'const obj = {',
