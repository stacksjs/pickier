...
describe('pickier plugin edge cases', () => {
  it('should handle sort-objects with different ordering options', async () => {
    ...
    writeFileSync(cfgPath, JSON.stringify({
      ...
      pluginRules: { 'sort-objects': ['warn', { type: 'line-length', order: 'desc' }], 'pickier/no-unused-vars': 'off' },
    }, null, 2), 'utf8')
  ...
  it('should handle sort-objects with case-insensitive sorting', async () => {
    ...
    writeFileSync(cfgPath, JSON.stringify({
      ...
      format: { extensions: ['ts'], trimTrailingWhitespace: true, maxConsecutiveBlankLines: 1, finalNewline: 'one', indent: 2, quotes: 'single', semi: false },
      rules: { noDebugger: 'off', noConsole: 'off' },
      pluginRules: { 'sort-objects': ['warn', { ignoreCase: true }], 'pickier/no-unused-vars': 'off' },
    }, null, 2), 'utf8')
  ...
  it('should handle sort-named-imports with line-length sorting', async () => {
    ...
    writeFileSync(cfgPath, JSON.stringify({
      ...
      format: { extensions: ['ts'], trimTrailingWhitespace: true, maxConsecutiveBlankLines: 1, finalNewline: 'one', indent: 2, quotes: 'single', semi: false },
      rules: { noDebugger: 'off', noConsole: 'off' },
      pluginRules: { 'sort-named-imports': ['warn', { type: 'line-length', order: 'asc' }], 'pickier/no-unused-vars': 'off' },
    }, null, 2), 'utf8')
