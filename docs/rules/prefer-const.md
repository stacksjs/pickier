...
-**Severity**: configurable via`pluginRules`-**Options**: none
...
Code that triggers the rule:```ts
const count = 1
function inc() {
  console.log(count)
}
...
```Code that does not trigger the rule:```ts
...
Enable the rule via`pluginRules`in your`pickier`config:```ts
// pickier.config.ts
export default {
  pluginRules: {
    'prefer-const': 'error',
